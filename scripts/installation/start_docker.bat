...
if /i "!run_tts!"=="y" (
    ...
    docker compose -f docker-compose.windows.yml !PROFILES! exec speaches uv tool run speaches-cli model download speaches-ai/Kokoro-82M-v1.0-ONNX
    echo [INFO] Downloading Faster Whisper Medium model ^(STT^)...
    docker compose -f docker-compose.windows.yml !PROFILES! exec speaches uv tool run speaches-cli model download Systran/faster-whisper-medium.en
